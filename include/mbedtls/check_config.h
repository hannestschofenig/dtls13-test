...
/* In TLS 1.3 we do not always need the ASN.1 routines. 
   For example, with a PSK ciphersuite and the use of 
   key shares we can build a ECDHE-based PSK ciphersuite
   that does not use any ASN.1 functionality. 
...
#endif
/* NOTE: There is no need to require X.509 parsing just because of SNI
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_TRUNCATED_HMAC)
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_ZLIB_SUPPORT)
...
#if defined(MBEDTLS_SSL_NEW_SESSION_TICKET) && defined(MBEDTLS_SSL_SESSION_TICKETS)
