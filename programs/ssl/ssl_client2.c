...
#define USAGE \
    ...
    "                        options: ssl3, tls1, tls1_1, tls1_2, dtls1, dtls1_2, tls1_3, dtls1_3\n" \
    "\n"                                                    \
...
int main( int argc, char *argv[] )
{
    ...
    if( strlen( opt.crt_file ) )
		if (strcmp(opt.crt_file, "none") == 0) {
			mbedtls_printf("- no client certificate being used\n");
            ret = 0;
		}
        else
...
#if defined(MBEDTLS_CERTS_C)
	{
		...
		mbedtls_printf("- loading test certificate\n");
	}
#else
    {
        ret = 1;
    ...
    if( strlen( opt.key_file ) )
		if (strcmp(opt.key_file, "none") == 0) {
			mbedtls_printf("- no private key for client configured\n");
			ret = 0;
		}
        else
...
#if defined(MBEDTLS_CERTS_C)
	{
		...
		mbedtls_printf("- loading test private key\n");
	}
#else
    {
        ret = 1;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_NEW_SESSION_TICKET)
		/* Here we need to decide when to send a ticket.
		 * 
