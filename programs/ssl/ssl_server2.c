...
#define USAGE_PSK                                                   \
    ...
    "    psk_identity=%%s     default: \"\"  (in hex, without 0x)\n" \
...
#define USAGE_TICKETS                                       \
    ...
    "    ticket_lifetime=%%d   default: 86400 (one day)\n"
#else
#define USAGE_TICKETS ""
...
#define USAGE_COOKIES \
    ...
    "                        0: disabled, 1: enabled, -1: library broken, 2: force return-routability check\n"
#else
#define USAGE_COOKIES ""
#endif
...
int main( int argc, char *argv[] )
{
	...
	unsigned char psk_identity[MBEDTLS_PSK_IDENTITY_MAX_LEN];
    ...
    opt.ticket_lifetime     = DFL_TICKET_LIFETIME;
#if defined(MBEDTLS_SSL_NEW_SESSION_TICKET)
    ...
    for( i = 1; i < argc; i++ )
    {
        ...
        *q++ = '\0';
        ...
        else if( strcmp( p, "ticket_lifetime" ) == 0 )
        {
            opt.ticket_lifetime = atoi( q );
            if( opt.ticket_lifetime < 0 )
                goto usage;
        }
#if defined(MBEDTLS_SSL_NEW_SESSION_TICKET)
    ...
    if( opt.alpn_string != NULL )
    {
        ...
        while(  ( (unsigned int) i < (sizeof( alpn_list) - 1)) && (*p != '\0' ) )
        {
            alpn_list[i++] = p;
	...
	if (opt.key_exchange_modes == KEY_EXCHANGE_MODE_ECDHE_ECDSA || opt.key_exchange_modes == KEY_EXCHANGE_MODE_ALL)
	{
		...
		if (strlen(opt.ca_path))
			...
			mbedtls_printf("- loading test CA certificates\n");
        ...
        strcmp( opt.key_file2, "none" ) != 0 )
    {
		...
		mbedtls_printf("- loading test server certificates\n");
    ...
    if( opt.tickets == MBEDTLS_SSL_SESSION_TICKETS_ENABLED )
    {
        if( ( ret = mbedtls_ssl_ticket_setup( &ticket_ctx,
                        ...
                        opt.ticket_lifetime, opt.ticket_flags ) ) != 0 )
        {
...
//    {
        ...
        if( opt.cookies == 0 ) // cookie support disabled
        {
...
#if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
	/*
	* We assume that the psk_identity is also provided as a hex value
	*/

	if (strlen(opt.psk_identity))
	{
		unsigned char c;
		size_t j;

		if (strlen(opt.psk_identity) % 2 != 0)
		{
			mbedtls_printf("psk identity not valid hex\n");
			goto exit;
		}

		psk_identity_len = strlen(opt.psk_identity) / 2;

		mbedtls_printf("\npsk identity:");

		for (j = 0; j < strlen(opt.psk_identity); j += 2)
		{
			c = opt.psk_identity[j];
			...
			psk_identity[j / 2] = c << 4;

			c = opt.psk_identity[j + 1];
			if (c >= '0' && c <= '9')
				c -= '0';
			else if (c >= 'a' && c <= 'f')
				c -= 'a' - 10;
			...
			mbedtls_printf("%x", psk_identity[j / 2]);
		}
	}
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    mbedtls_printf( "    [ Maximum fragment length is %d ]\n",
...
data_exchange:
     ...
     */
	if (--exchanges_left > 0) {
		mbedtls_printf(" Continue doing data exchange\n");
		goto data_exchange;
	}
    /*
